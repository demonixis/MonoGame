...
if [[ ! -f "$android_ndk_root/build/cmake/android.toolchain.cmake" ]]; then
    echo "Android NDK $android_ndk_version was not found. Set MONOGAME_ANDROID_NDK_ROOT." >&2
    exit 1
fi

script_directory="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
build_directory="$script_directory/../obj/android-opengles/$build_configuration"
